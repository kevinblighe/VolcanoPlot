...
'cutoffLineWidth', respectively. To disable the lines, set either
cutoffLineType="blank" or cutoffLineWidth=0.

Also, by using pCutoffCol='padj', we instruct *EnhancedVolcano* to generate
a traditonal volcano using nominal / un-adjusted p-values (or whatever is
passed as 'y'), but using adjusted p-values as the cut-off.

Extra lines can also be added via 'hline' and 'vline' to display other custom
cut-offs. To make these more visible, we will also remove the default gridlines
(gridlines.major and gridlines.minor).


```{r ex5, fig.height = 8.5, fig.width = 7, fig.cap = "Adjust cut-off and add extra threshold lines."}
...
```{r ex6, eval = FALSE, fig.height = 8.5, fig.width = 10, fig.cap = "Adjust legend position, size, and text."}
